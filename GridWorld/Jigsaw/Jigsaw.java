...
public class Jigsaw {
	...
	private void sortedInsertOpenList(JigsawNode jNode) {
		...
		for (int i = 0; i < this.openList.size(); i++) {
			if (jNode.getEstimatedValue() < this.openList.elementAt(i).getEstimatedValue()) {
	...
	/**（Demo+实验二）启发式搜索。访问节点数大于30000 ( 25000 ) 个则认为搜索失败。
		...
		for (int i = 1; i <= dimension * dimension; i++) {
			for (int j = 1; j <= dimension * dimension; j++) {
				if (jNode.getNodesState()[i] != 0 && jNode.getNodesState()[i] == endJNode.getNodesState()[j]) {
					...
					distance += x + y;
					geometry += (int) Math.sqrt(x * x + y * y);
					break;
				}
			}
		}
		int value = distance * 2 + s * 1 + geometry * 1;
